...
import (
	"errors"
	"fmt"
	"github.com/kevin0120/GoScrewdriverWebApi/utils/keyvalue"
	"log"
	"runtime"
	"time"
)
...
func (h *HTTPDHandler) NewAccessLogger() *log.Logger {
	s := &StaticLevelHandler{
		l:     h.l.With(String("services", "httpd_access_log")),
		level: llDebug,
	}
...
func (h *HTTPDHandler) StartingService() {
	h.l.Info("starting HTTP services")
}

func (h *HTTPDHandler) StoppedService() {
	h.l.Info("closed HTTP services")
}
